...
plan_from_vbl <- read_yaml(plan_yaml_vbl)
str(plan_from_vbl)
```
...
The same `read_yaml()` function reads in a to-do YAML file and converts it to an R list:

```{r}
todo_path <- system.file("extdata", "todo.yml", package = "tidytracker", mustWork = TRUE)
my_todo <- read_yaml(todo_path)
str(my_todo)
```
