...
plan_from_vbl <- read_plan_todo_yaml(plan_yaml_vbl)
str(plan_from_vbl)
```

Should you ever forget exactly what this YAML should look like, the `template_plan_todo_yaml()` function is here to help. It prints a template example to your console:
...
The same `read_plan_todo_yaml()` function reads in a to-do YAML file and converts it to an R list:
...
Just like with plans, you can also pass a character string instead of a file and use `template_plan_todo_yaml()` to remind yourself of the required format: 
